...
export const useDiscount = () => {
     ...
     */
    const validateCoupon = async (code: string): Promise<{ valid: boolean; discountPercentage?: number; message?: string }> => {
        loading.value = true
        error.value = null
     ...
     */
    const calculateCartDiscounts = async (items: any[], couponCode?: string): Promise<DiscountCalculationResult | null> => {
        loading.value = true
        error.value = null
     ...
     */
    const calculateSavingsPercentage = (subtotalBeforeDiscount: number, discountTotal: number): number => {
