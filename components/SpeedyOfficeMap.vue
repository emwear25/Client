...
interface Office {
  ...
  address: {
    ...
    y?: number; // latitude
    location?: {
      // For compatibility with Econt structure
      latitude: number;
      longitude: number;
    };
  };
...
const officesByCity = computed((): CityGroup[] => {
  ...
  cityMap.forEach((city) => {
    ...
    if (officesWithCoords.length > 0) {
      const avgLat =
        officesWithCoords.reduce((sum, o) => sum + (o.address.y || 0), 0) /
        officesWithCoords.length;
      const avgLng =
        officesWithCoords.reduce((sum, o) => sum + (o.address.x || 0), 0) /
        officesWithCoords.length;
...
const fetchOffices = async () => {
  try {
    ...
    if (response.success && response.data) {
      ...
      console.log(
        "[SpeedyMap] Cities found:",
        new Set(offices.value.map((o: any) => o.address.city.name)).size
      );
    } else {
